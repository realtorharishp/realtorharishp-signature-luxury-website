...
import './style.css?v=1.0.1' // Changed to force cache clear
...
function App() {
  const properties = [
    ...
    { number: 2, status: 'Residential Sale', title: '14839 Story Lane', city: 'Frisco, TX 75035', price: '$923,631', image: '/property-2.jpg', facts: ['4 Bed', '3.5 Bath', '3,593 Sq Ft', 'Built 2011', '3 Car Garage', 'Dedicated Office', 'Dining Room', 'Media & Game Room'] },
  ...
  return (
    <div>
      <header className="navbar">
        <a href="#home" className="brand"><img src="/rek-logo.png" alt="REKonnection Logo" /></a>
        <nav><a href="#home">Home</a><a href="#listings">Listings</a><a href="#services">Services</a><a href="#about">About</a><a href="#contact">Contact</a></nav>
      ...
      <section id="home" className="hero">
        <div>
          ...
          <p className="subtitle">Luxury real estate representation across North Texas.</p>
          <div className="buttons"><a href="#listings" className="btnGold">View Listings</a><a href="#contact" className="btnOutline">Consultation</a></div>
          <div className="agent"><img className="agentImage" src="/harish-patel.png" alt="Harish Patel" /><div><h3>Harish Patel</h3><p>Realtor® • 972-552-0158</p></div></div>
        </div>
        <div className="heroCard"><div className="logoPanel"><img src="/rek-logo.png" alt="REKonnection" /></div></div>
      </section>
        ...
        <div className="propertyGrid">
          {properties.map((p) => (
            <article className="propertyCard" key={p.number}>
              <div className="imageWrap"><img src={p.image} alt={p.title} /><div className="numberBadge">{p.number}</div><div className="statusBadge">{p.status}</div></div>
              <div className="propertyBody"><p className="propertyPrice">{p.price}</p><h3>{p.title}</h3><p className="propertyCity">{p.city}</p><div className="facts">{p.facts.map((f) => <span key={f}>{f}</span>)}</div><a className="propertyButton" href="#contact">Schedule Showing</a></div>
            </article>
          ))}
        </div>
      </section>
      ...
      <section id="contact" className="contact">
        ...
        <form action="https://api.web3forms.com/submit" method="POST">
          ...
          <input type="text" name="name" placeholder="Name" required /><input type="text" name="phone" placeholder="Phone" required /><input type="email" name="email" placeholder="Email" required /><textarea name="message" placeholder="Your Goals" required></textarea>
          <button type="submit">Send Message</button>
        </form>
      </section>

      <footer><img src="/rek-logo.png" alt="REKonnection" /><p>Harish Patel • Realtor® • 972-552-0158</p></footer>

      <div className="floatingButtons"><a href="tel:9725520158">📞</a><a href="sms:9725520158">💬</a><a href="mailto:realtor.harishp@gmail.com">📧</a></div>
